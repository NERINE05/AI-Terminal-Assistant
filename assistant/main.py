...
def load_config() -> Cfg:
    """Load config.yaml and resolve paths relative to the package."""
    # Load config
    try:
    ...
    if "model_path" not in llm_cfg:
        # Model path inside the package
        llm_cfg["model_path"] = str(importlib.resources.files("assistant").joinpath("models/mistral-7b-instruct.Q4_K_M.gguf"))
    llm_cfg.setdefault("provider", "gpt4all")
